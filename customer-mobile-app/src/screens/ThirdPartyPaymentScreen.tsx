...
const ThirdPartyPaymentScreen = ({ route, navigation }: any) => {
  ...
  const [processing, setProcessing] = useState<"success" | "failed" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (!orderId || !sessionId) {
      console.error("[ThirdPartyPayment] Missing required params:", {
        orderId,
        sessionId,
      });
  ...
  const handlePayment = async (status: "success" | "failed") => {
    ...
    if (!orderId || !sessionId) {
      console.error(
        "[ThirdPartyPayment] Cannot process payment - missing orderId or sessionId"
      );
      setError("Thông tin thanh toán không hợp lệ");
      return;
    }

    console.log("[ThirdPartyPayment] Processing payment:", {
      orderId,
      sessionId,
      status,
    });
    ...
    try {
      ...
      const response = await paymentAPI.confirmThirdParty({
        orderId,
        sessionId,
        status,
      });
      ...
      if (status === "success") {
        console.log(
          "[ThirdPartyPayment] Payment success - clearing cart and navigating"
        );
        try {
  ...
  return (
    <SafeAreaView
      style={[styles.safeArea, { backgroundColor: config.bgColor }]}
    >
      <View style={[styles.container, { backgroundColor: config.bgColor }]}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>
              {restaurantName || "FoodFast Partner"}
            </Text>
          </View>
          ...
          {expiresText && (
            <View style={styles.expiryNotice}>
              <Text style={styles.expiryText}>
                ⏰ Phiên thanh toán hết hạn lúc {expiresText}
              </Text>
            </View>
          )}
        </View>
        ...
        <TouchableOpacity
          style={[
            ...
            processing === "success" && styles.disabledButton,
          ]}
        ...
        <TouchableOpacity
          style={[
            styles.cancelButton,
            processing === "failed" && styles.disabledButton,
          ]}
          disabled={processing !== null}
