...
const ProductDetailScreen = ({ route, navigation }: any) => {
  ...
  const images = useMemo(() => {
    ...
    if (Array.isArray(product.images)) {
      collected.push(
        ...product.images.filter((item: any) => typeof item === 'string' && item.trim().length > 0)
      );
    }
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
            ...
            return (
              <React.Fragment key={idx}>
                <Image
                  ...
                  resizeMode="cover"
                  onError={() => {
                    if (!failedImageIndexes[idx] && displayUri !== PLACEHOLDER_IMAGE) {
                      console.error('Hero onError for index', idx, 'uri', displayUri);
                      setFailedImageIndexes(prev => ({ ...prev, [idx]: true }));
                    }
                  }}
                  onLoadEnd={() => console.log('Hero onLoadEnd index', idx, 'uri', displayUri)}
                />
                {DEBUG_IMAGES && (
                  <Text style={{ fontSize: 10, color: '#888', paddingHorizontal: 8, marginTop: 6 }} numberOfLines={1} ellipsizeMode="middle">{displayUri}</Text>
                )}
              </React.Fragment>
            );
          })}
        ...
        <View style={styles.card}>
          ...
          <View style={[styles.row, { marginTop: 12 }]}> 
            <View>
              <Text style={styles.price}>{String(price.toLocaleString('vi-VN'))}đ</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6 }}>
                {/* Rating and reviews count */}
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  {Array.from({ length: 5 }).map((_, i) => {
                    const iconName = (reviewSummary.averageRating ?? 0) >= i + 1 ? 'star' : (reviewSummary.averageRating ?? 0) >= i + 0.5 ? 'star-half' : 'star-outline';
                    return (
                      <Ionicons key={i} name={iconName as any} size={14} color="#FFB800" style={{ marginRight: 4 }} />
                    );
                  })}
                  <Text style={{ marginLeft: 6, fontWeight: '600' }}>{String((reviewSummary.averageRating ?? 0).toFixed(1))}</Text>
                  <Text style={{ marginLeft: 6, color: '#777' }}>({String((reviewSummary.totalReviews ?? 0).toLocaleString('vi-VN'))})</Text>
                </View>
              </View>
            </View>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              ...
              <Text style={{ fontWeight: '700', fontSize: 16 }}>{String((product.sold ?? product.sales ?? 0).toLocaleString('vi-VN'))}</Text>
            </View>
          </View>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            {loadingReviews ? (
              <View style={styles.productReviewLoading}>
                ...
                <Text style={styles.productReviewLoadingText}>Đang tải đánh giá...</Text>
              </View>
            ) : reviews.length === 0 ? (
              <Text style={styles.productReviewEmpty}>Chưa có đánh giá cho món ăn này.</Text>
            ) : (
              reviews.map((c: any) => (
                <View
                  key={c._id || c.id}
                  style={styles.productReviewItem}
                >
                  <View style={styles.productReviewHeader}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.productReviewName}>{c.customer?.name || c.user || 'Ẩn danh'}</Text>
                      <View style={styles.productReviewStarRow}>
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Ionicons
                            key={i}
                            name={i < Math.round(c.rating ?? 0) ? 'star' : 'star-outline'}
                            size={12}
                            color="#FFB800"
                          />
                        ))}
                        <Text style={styles.productReviewDate}>{formatReviewDate(c.createdAt || c.date)}</Text>
                      </View>
                    </View>
                  </View>
                  <Text style={styles.productReviewComment}>{c.comment || c.text}</Text>
                </View>
              ))
            )}
          </View>
      ...
      <View style={styles.floatingBar}>
        <View>
          ...
          <Text style={styles.totalPrice}>{String(price.toLocaleString('vi-VN'))}đ</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  productReviewLoading: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12 },
  productReviewLoadingText: { marginLeft: 10, color: '#777' },
  productReviewEmpty: { color: '#777', fontStyle: 'italic' },
  productReviewItem: { paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  productReviewHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  productReviewAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#eee', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  productReviewAvatarText: { fontWeight: '700', fontSize: 16, color: '#333' },
  productReviewName: { fontWeight: '700', color: '#333' },
  productReviewStarRow: { flexDirection: 'row', alignItems: 'center', marginTop: 4 },
