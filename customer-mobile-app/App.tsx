...
const deriveInitialPosition = (width: number, height: number) => {
  ...
  const x = Math.max(
    HORIZONTAL_MARGIN,
    width - BUTTON_SIZE - HORIZONTAL_MARGIN
  );
  const y = Math.max(
    TOP_SAFE_MARGIN,
    height - BUTTON_SIZE - BOTTOM_SAFE_MARGIN
  );
  return { x, y };
};
...
function CartFloatingButton() {
  ...
  const [position, setPosition] = useState<{ x: number; y: number }>(() =>
    deriveInitialPosition(width, height)
  );
    ...
    const loadUser = async () => {
      try {
        ...
        if (userJson && token) {
          ...
          try {
            await (store.dispatch(fetchCart()) as any).unwrap();
          } catch (error: any) {
      ...
      <View style={{ flex: 1 }}>
        <Stack.Navigator screenOptions={{ headerShown: false }}>
          ...
          <Stack.Screen
            ...
            component={ThirdPartyPaymentScreen}
          ...
          <Stack.Screen
            ...
            options={{ headerShown: true, title: "Cấu hình Server" }}
          />
        </Stack.Navigator>
        <CartFloatingButton />
      </View>
